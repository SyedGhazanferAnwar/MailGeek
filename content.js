...
function myfunction(){
 	var email_container=document.getElementsByClassName("gb_kb");//for my email
 		...
 		setTimeout(function(){
 			...
 			for (var i = 0; i<conts.length; i++) {
	 			...
	 			if(to_class_afn==="To:"){
					...
					$.ajax({
						url: "https://geekacode2.000webhostapp.com/checker.php",
						type: "POST",
						data: formData2,
						processData: false,
						contentType: false,
...
sub_id=sub_id_block2[0].innerHTML; //Subject Extracted
...
{
	a[0].innerHTML='<img alt="MailGeek" src='+"https://geekacode2.000webhostapp.com/images/"+filenamea+' style="display:none">'+text;
...
k[1].onclick=function()
{
	...
	if(a.length>0){
		...
		$.ajax({
			url: "https://geekacode2.000webhostapp.com/index.php",
			type: "POST",
			data: formData,
			processData: false,
			contentType: false,
